# ProstateMicroSeg/scripts/run_train_3d_v3.py
...
from src.utils.visualization import save_val_case_slice_grid_png
...
def main() -> None:
    ...
    val_patch_ds = MicroUS3DPatchDatasetV2(
        dataset_root=args.data_root,
        splits_dir=args.splits_dir,
        split="val",
        target_zyx=patch_zyx,
    ...
    train_sampler = OversampleForegroundBatchSampler3DV2(
        dataset=train_ds,
    ...
    val_patch_sampler = OversampleForegroundBatchSampler3DV2(
    ...
    for epoch in range(start_epoch, start_epoch + int(args.epochs)):
        ...
        valp_metrics = validate_patches_3d(
            ...
            criterion=criterion,
            device=device,
            dice_soft_fn=lambda lg, y: dice_soft_from_logits(
    ...
    batch = next(iter(val_case_loader))  # one full validation volume
    img = batch["image"].to(device)
    lbl = batch["label"].to(device)
    case_id = batch.get("case_id", "valcase0")

    vis_dir = run_dir / "vis"
    out_png = vis_dir / f"final_{case_id}.png"

    dice2d = save_val_case_slice_grid_png(
        model=model,
        img=img,
        lbl=lbl,
        out_png=out_png,
